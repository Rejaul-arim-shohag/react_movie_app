...
export default function CartDetails({ onClose }) {
  ...
  const handeRemoveItem = (event, item_Id) => {
    event.preventDefault();
    // const filterData = cartData.filter((item) => item.id !== item_Id);
              ...
              cartData.map((item) => (
                <div key={item.id} className="grid grid-cols-[1fr_auto] gap-4">
                  <div className="flex items-center gap-4">
                    <img className="rounded overflow-hidden" width="50px" height="50px" src={getImgUrl(item.cover)} alt="" />
                    <div>
                  ...
                  <div className="flex justify-between gap-4 items-center">
                    <button
                      onClick={(e) => handeRemoveItem(e, item.id)}
